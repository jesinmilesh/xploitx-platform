...
class ServerConfig(object):
    ...
    if not DATABASE_URL:
        if empty_str_cast(config_ini["server"]["DATABASE_HOST"]) is not None:
            ...
            src_db = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ctfd.db")
            tmp_db = os.path.join(tmp_dir, "ctfd.db")
    ...
    if CACHE_REDIS_URL:
        ...
        CACHE_DIR: str = os.path.join(tempfile.gettempdir(), "cache")
    ...
    LOG_FOLDER: str = empty_str_cast(config_ini["logs"]["LOG_FOLDER"]) \
        or (os.path.join(tempfile.gettempdir(), "logs") if os.getenv("VERCEL") else os.path.join(os.path.dirname(os.path.abspath(__file__)), "logs"))
    ...
    UPLOAD_FOLDER: str = empty_str_cast(config_ini["uploads"]["UPLOAD_FOLDER"]) \
        or (os.path.join(tempfile.gettempdir(), "uploads") if os.getenv("VERCEL") else os.path.join(os.path.dirname(os.path.abspath(__file__)), "uploads"))
    ...
    SERVER_SENT_EVENTS: bool = False if os.getenv("VERCEL") else process_boolean_str(empty_str_cast(config_ini["optional"]["SERVER_SENT_EVENTS"], default=True))
